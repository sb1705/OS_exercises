...
while(o_scan=readdir(FD)){

  if(!strcmp(get_ext(o_scan->d_name),"o")){//if a file.o is found

    filename=o_scan->d_name;
    printf("Filename.o prima del ciclo: %s\n",filename);
		while(c_scan=readdir(FD2)){

      printf("Cerco il .c in: %s\n",c_scan->d_name);
			if((!strncmp(filename, c_scan->d_name, strlen(c_scan->d_name)-2)) && (!strcmp(get_ext(c_scan->d_name), "c"))){

          //if a file.c is found
			    stat(filename,&s1);
          time1= s1.st_mtime; //last mod file.o
          printf("Filename: %s\n",filename);
 ...
 if(FD2==NULL){
    ...
    if(!found) myexec(myArgv);
    else  printf("run genobj first\n");
